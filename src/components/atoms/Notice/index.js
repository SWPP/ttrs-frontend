  ...
  static getDerivedStateFromProps(props, state) {
    ...
    props.notices.forEach(notice => {
      if (state.lastId < Math.abs(notice.id)) {
        setTimeout(() => props.onHideNotice(notice.id), notice.duration ? notice.duration : 2000)
        setTimeout(() => props.onDismissNotice(notice.id), notice.duration ? notice.duration + 500 : 2500)
      ...
      <div>
        {this.props.notices.map(notice => (
          <Transition visible={!notice.invisible} key={notice.id} transitionOnMount>
            <Message
              style={{ marginLeft: 10, marginRight: 10, left: '50%', transform: 'translateX(-50%)', top: '10%', position: 'fixed', zIndex: Math.abs(notice.id) }}
              success={notice.id > 0}
